...
export function SwirlCanvas() {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      particles.forEach((p, idx) => {
        ...
        const newAge = age + 1;
        const alpha = 255 * triangleWave(newAge, ttl);
        ...
        particles.set([nx, ny, nvx, nvy, newAge, 0, ttl, vc, r, g, b], idx);

        const ix = x | 0;
        const iy = y | 0;
        if (ix >= 0 && ix < width && iy >= 0 && iy < height) {
          const lifeRatio = newAge / ttl;
          const speedFactor = Math.min(
            0.08 * Math.sqrt(nvx * nvx + nvy * nvy),
            1
          );
          ...
          const R = Math.min(
            ...
            Math.max(
              0,
              Math.floor(r * (1 + 0.3 * blend) + extra + 18 * speedFactor)
            )
          );
          const G = Math.min(
            255,
            Math.max(
              0,
              Math.floor(g * (1 + 0.15 * blend) + extra + 6 * speedFactor)
            )
          );
          const B = Math.min(
            255,
            Math.max(
              0,
              Math.floor(b * (1 + 0.1 * blend) + 3 * speedFactor)
            )
          );
  ...
  return (
    <canvas
      ref={canvasRef}
      className="fixed inset-0 h-full w-full pointer-events-none -z-10"
